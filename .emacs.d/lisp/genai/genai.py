...
def update_human_history(
    human_history, history_file, template_type, prompt, model_out
):
    ...
    mngs_io_save(human_history, history_file, verbose=False)


def update_ai_history(ai_history, ai_history_file, model):
    n_new_history = 2
    for history in model.history[-n_new_history:]:
        ai_history.append(history)
    mngs_io_save(ai_history, ai_history_file, verbose=False)


# def extract_history_to_feed(ai_history, n_history):
#     ai_history_to_feed = ai_history[-n_history - 1 :]

#     if ai_history_to_feed:
#         while ai_history_to_feed[0]["role"] != "user":
#             ai_history_to_feed = ai_history_to_feed[1:]

#         while ai_history_to_feed[-1]["role"] != "user":
#             ai_history_to_feed = ai_history_to_feed[:-1]

#     return ai_history_to_feed
...
def run_genai(
    ...
    # Handle histories
    human_history = mngs_io_load(history_file)
    ai_history_file = history_file.replace("history", "history-ai")
    ai_history = mngs_io_load(ai_history_file)
    ...
    update_ai_history(ai_history, ai_history_file, model)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--history_file",
        type=str,
        default=mngs_path_split(__file__)[0] + "./history-secret.json",
        help="(default: %(default)s)",
    )
